...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, labelClassName, error, rightIcon, className = "", containerClassName = "", id, type = "text", ...props }, ref) => {
    ...
    return (
      <div className={`flex flex-col w-full ${containerClassName}`}>
        {label && (
          <label
            htmlFor={inputId}
            className={cn("text-primary text-[16px] md:text-[20px] font-[500] font-inter mb-2 tracking-wide", labelClassName)}
          >
            {label}
          </label>
        )}
